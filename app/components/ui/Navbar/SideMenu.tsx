...
const SideMenu: FC<SideMenuProps> = ({ setShowSide }: SideMenuProps) => {
    return (
        <motion.div
            ...
            exit={{ opacity: 0 }}
            className='md:hidden bg-black h-screen w-screen fixed top-0 left-0 z-[30] px-[5%] flex flex-col justify-center items-end gap-10'>
            <div className='flex flex-col text-[20px] gap-5 tracking-[4px] font-[300] text-white text-end'>
                <Link href="/portfolio" onClick={() => { setShowSide(false) }} ><span className='underline-hover-effect relative' >Home</span></Link>
